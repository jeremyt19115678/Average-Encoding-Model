"""
A slightly modified version of Zijin's implementation of AlexNet: https://github.com/zijin-gu/NeuroGen/blob/main/torchmodel/models/alexnet.py
"""
import torch
...
class AlexNet(nn.Module):
    ...
    def forward(self, x):
        ...
        f7 = self.fc7(f6)
        f8 = self.fc8(f7)
        return [c1, c2, c3, c4, c5, y, f6[:, :, None, None], f7[:, :, None, None], f8[:, :, None, None]] # changed from Zijin's Code
...
class Alexnet_fmaps(nn.Module):
    ...
    '''
    def __init__(self): # changed from Zijin's Code
        super(Alexnet_fmaps, self).__init__()
        # two lines from Zijin's Code omitted
        self.extractor = build_alexnet_fmaps(pretrained=True)

    def forward(self, _x):
        return self.extractor(_x) # changed from Zijin's Code